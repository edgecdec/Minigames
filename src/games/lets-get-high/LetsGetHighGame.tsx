...
function Bob({ mood }: { mood: "idle" | "thinking" | "happy" | "sad" }) {
  ...
  return (
    <Box className={`bob bob-${mood}`} role="img" aria-label="Bob the pixel guide" sx={{ textAlign: "center", animation: mood === "sad" ? "none" : "bobFloat 1.6s ease-in-out infinite", "@keyframes bobFloat": { "0%, 100%": { transform: "translateY(0)" }, "50%": { transform: "translateY(-6px)" } } }}>
      <Box component="pre" sx={{ m: 0, fontFamily: "monospace", fontWeight: 800, lineHeight: 1.05, fontSize: { xs: "1.15rem", sm: "1.4rem" } }}>
        {face}
      </Box>
      <Typography variant="caption" sx={{ color: "text.secondary", fontWeight: 700 }}>BOB{mood === "thinking" ? " •••" : ""}</Typography>
    </Box>
  );
}
...
export default function LetsGetHighGame() {
  ...
  const share = async () => {
    const text = `I got ${bestNumber} in Let's Get High! Can you go higher?`;
  ...
  return <>
    <ScoreBar stats={[{ label: "Raises", value: state.rounds }, { label: "Best raises", value: best, muted: !bestLoaded }, { label: "Biggest", value: bestNumberLoaded ? bestNumber : "…", muted: !bestNumberLoaded }]} />
    <Stack spacing={2.5} alignItems="center" sx={{ width: "100%", py: 1 }}>
      ...
      {state.status === "playing" && <Typography className="huge-number" sx={{ width: "100%", textAlign: "center", fontWeight: 900, lineHeight: 1.1, color: "primary.main" }}>{state.current.toString()}</Typography>}
      {lost && <Typography color="text.secondary" sx={{ textAlign: "center" }}>You entered {state.lastAnswer?.toString()}. Bob’s number was {state.current.toString()}.</Typography>}
